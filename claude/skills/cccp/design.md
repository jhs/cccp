...
- **Large-file publish (>256 MB):** single-PUT only. The tool detects oversize
  ...
  uploader.
- **Poll cadence** is now an adaptive ladder (2s, 5s, 15s, 30s, 1m, 2m, 3m, 5m;
  activity resets to 2s; `cccp wake` short-circuits the gap via SIGUSR1).
  Replaced the prior fixed 0.25s, which was a holdover from v1's local-FS scan
  and meant ~$1.90/day per always-on comrade in Azure LIST cost. Future
  improvement captured in TODO: per-session pidfile so `cccp wake` signals
  exactly one watchtower instead of every sibling on the same host+user.
