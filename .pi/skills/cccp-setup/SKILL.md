---
name: cccp-setup
description: Inspect, choose, configure, or troubleshoot the CCCP data backend, or read/set any CCCP config value. Use for "which cccp backend am I on", "set up cccp on azure", "switch cccp to local-fs", "why can't cccp reach the hub", "what cccp settings do I have", or after CCCP announces a freshly created data directory. NOT for joining cells or messaging — use the cccp-chat skill for that.
---
...
Respond to your user's request by understanding the CCCP plugin's **data
backend** system, so that you are able to check the current backend, change to
a different backend, configure a new backend, or reconfigure an existing
backend. With no specific request, run `cccp config` and report what you find.

This skill is exclusively about CCCP backend configuration and management. If
the user wants to chat, join a cell, send a message, etc., point them at the
cccp-chat skill and stop.

The `cccp` CLI is on your bash tool's PATH from session start — the
cccp-comrade extension puts it there — and `CCCP_PLUGIN_DATA` is already
exported: your machine's Claude plugin data directory when one exists (a shared
store — same-machine Claude and Pi comrades reach the same local cells), else
`~/.pi/cccp`, auto-created on first run.
...
| `azure-blob` | Centralized cloud storage (Azure Blob), reachable from any host, user, or network. Low cost. Auth is a shared container-scoped SAS token. | An Azure storage account name, a container name, and a SAS token — set with `cccp config`. | Operate an Azure Blob container: either deploy the repo's Terraform reference (`infra/azure/apply.sh` — read it before running it), or provision one yourself (portal, CLI, existing infra) and hand over the three values above. Needs an Azure subscription and may spend the user's money. |
...
(Under Pi, the cccp-comrade extension exports it at session start.)
...
`Inactive backend:` line — an inactive backend keeps its stored config, which
is the whole setup flow: configure it while inactive, `cccp backend use` it
when ready.
...
switching) and `CCCP_PLUGIN_DATA` (environment-only; the cccp-comrade
extension exports it at session start).
...
- use the **absolute path** to the cccp binary; bare `cccp` is on `$PATH` only
  inside a session running the cccp extension
...
```bash
CCCP_PLUGIN_DATA=/home/u/.pi/cccp \
  /path/to/bin/cccp \
...
Individual projects may want dissimilar CCCP setups — a different backend, or
the same backend with different settings (a per-project prefix, a per-client
account, a shared local-fs root). Every config value is ultimately an
environment variable and env is the highest-precedence layer, so this needs no
cccp feature — just the right `CCCP_*` vars exported in that project's
sessions before the extension resolves defaults at session start.
