...
The Captain sections above cover staffing authority and lifecycle decisions. This is the mechanical HOW: spawning, observing, and terminating comrades as tmux windows via `spawn-comrade` (on your `$PATH`).
...
```
spawn-comrade -m <model> -e <effort> [--captain @@COMRADE_ID@@] <Name> <slug> [docs...]
```
...
**The Captain holds window 0.** Under tmux's default `base-index 0`, that makes `<session>:0` the cell's coordinator no matter who currently holds the role — one fixed place to look. Indices are incidental everywhere else; the window *name* is the stable handle, and the swap in step 3 is what keeps the slot across a succession.

1. **Rename your window first:** `tmux rename-window -t Captain CaptainEmeritus`
   Frees the name `Captain` for the spawn. You still hold window 0.
2. **Spawn the successor:** `spawn-comrade -m <model> -e <effort> --skill captain-with-tmux Captain <slug>`
   It lands at a nonzero index, since 0 is still yours.
3. **Swap it into window 0:** `tmux swap-window -d -s Captain -t CaptainEmeritus`
   By name: `Captain` moves to index 0, `CaptainEmeritus` to the successor's old index. `-d` leaves focus where it is.
