...
per-client credentials, "our team's cell" — **offer this proactively** rather
than editing global config, and read `references/per-project.md` for the
reasons and the reference implementation (committed `.cccp.env`, gitignored
secrets sibling, a project-scoped SessionStart hook writing to
`$CLAUDE_ENV_FILE`).
